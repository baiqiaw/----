...
- **开发环境**：SQLite 3（无需单独安装，Prisma 自动管理）
- **个人版（桌面客户端）**：SQLite 3（本地文件存储）
- **企业版（Web服务/生产环境）**：PostgreSQL 14+
...
- 使用 Prisma ORM 可以无缝切换数据库，代码层面无需修改
- SQLite 和 PostgreSQL 的兼容性处理见下文
...
**PostgreSQL 枚举类型**（企业版使用）：

PostgreSQL 支持原生 ENUM 类型，用于固定且不经常变化的枚举值：
...
**SQLite 兼容性处理**（开发环境 + 个人版）：

SQLite 不支持 ENUM 类型，需要使用以下方案之一：

1. **方案一：VARCHAR + CHECK 约束**（推荐，Prisma 自动处理）

   ```sql
   -- Prisma 会自动将 enum 转换为 VARCHAR + CHECK 约束
   CREATE TABLE users (
       role VARCHAR(20) NOT NULL CHECK (role IN ('admin', 'enterprise_admin', 'enterprise_user', 'basic_user', 'free_user')),
       ...
   );
   ```

2. **方案二：使用字典表**（已在设计中）
   - 使用 `file_formats`、`audit_actions`、`resource_types` 等字典表
   - 通过外键关联，更灵活，便于扩展

**Prisma Schema 兼容性处理**：

Prisma 会自动处理数据库差异，在 Schema 中使用 `enum` 类型，Prisma 会根据数据库类型自动转换：
...
- **PostgreSQL**：Prisma 会生成 PostgreSQL ENUM 类型
- **SQLite**：Prisma 会生成 VARCHAR 类型 + CHECK 约束

### 3.2 PostgreSQL 枚举类型定义（企业版）

以下枚举类型定义适用于 PostgreSQL 14+：
...
| TEXT | TEXT | 完全兼容 |
| INTEGER | INTEGER | 完全兼容 |
| BIGINT | INTEGER | SQLite 使用 INTEGER 存储 |
| TIMESTAMP | TEXT / INTEGER | SQLite 使用 TEXT（ISO8601）或 INTEGER（Unix时间戳） |
| BOOLEAN | INTEGER | SQLite 使用 0/1 |
| JSONB | TEXT | SQLite 使用 TEXT + JSON 函数 |
| ENUM | TEXT + CHECK | SQLite 使用 VARCHAR + CHECK 约束 |
| ARRAY | TEXT | SQLite 使用 JSON 数组或逗号分隔字符串 |

### 8.2 功能兼容性

| 功能 | PostgreSQL | SQLite | 处理方案 |
| ---- | ---- | ---- | ---- |
| ENUM 类型 | ✅ 支持 | ❌ 不支持 | 使用 VARCHAR + CHECK 约束 |
| JSONB 类型 | ✅ 支持 | ❌ 不支持 | 使用 TEXT + JSON 函数 |
| 数组类型 | ✅ 支持 | ❌ 不支持 | 使用 JSON 数组或逗号分隔字符串 |
| 全文搜索 | ✅ 支持（GIN索引） | ⚠️ 有限支持 | 使用 LIKE 或 FTS5 扩展 |
| 并发控制 | ✅ 优秀 | ⚠️ 有限 | SQLite 适合单用户或低并发场景 |
| 外键约束 | ✅ 支持 | ✅ 支持（需启用） | 完全兼容 |
| 事务 | ✅ 支持 | ✅ 支持 | 完全兼容 |

### 8.3 Prisma 自动处理

Prisma ORM 会自动处理大部分兼容性问题：

- **ENUM 类型**：自动转换为 VARCHAR + CHECK 约束（SQLite）
- **JSONB 类型**：自动转换为 TEXT + JSON 函数（SQLite）
- **数组类型**：自动转换为 JSON 数组（SQLite）
- **时间戳**：自动处理时区和格式差异
- **布尔值**：自动转换为 INTEGER（SQLite）

**注意事项**：

1. **开发环境**：使用 SQLite，简化配置，提高开发效率
2. **个人版**：使用 SQLite，无需额外数据库服务，数据存储在本地
3. **企业版**：使用 PostgreSQL，获得更好的性能和功能支持
4. **迁移**：使用 Prisma 可以轻松在 SQLite 和 PostgreSQL 之间切换

---

## 9. 性能优化

### 9.1 分区策略（仅 PostgreSQL）

对于大数据量表，考虑使用分区：

```sql
-- 转换历史表按月分区（仅 PostgreSQL 支持）
